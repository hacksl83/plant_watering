...
struct _custom_chars {
	
	uint8_t _custom5x7[4][PCD8544_CHAR5x7_WIDTH] ;
	uint8_t _custom3x5[1][PCD8544_CHAR3x5_WIDTH];
};

const __flash struct _custom_chars custom_chars= {
	{	
		{ 0x00, 0x00, 0x00, 0x00, 0x00 },
		{ 0x3E, 0x22, 0x22, 0x22, 0x3E },	// rectangle not filled 
		{ 0x3E, 0x3E, 0x3E, 0x3E, 0x3E }, 	// rectangle filled
...
//Fonts 5x7
const __flash uint8_t PCD8544_Font5x7 [97][PCD8544_CHAR5x7_WIDTH] = {
	...
	{ 0x7F, 0x7F, 0x7F, 0x7F, 0x7F },   // Stop			127
	{ 0x00, 0x7F, 0x3E, 0x1C, 0x08 },    // Play				128

		
};

const __flash uint8_t PCD8544_Font3x5[106][3] = {
...
void PCD8544_InitIO(void) {
	...
	//DC pin as output
	PCD8544_DC_DDR |= 1 << PCD8544_DC_PIN;
	
	//CE pin as output
	PCD8544_CE_DDR |= 1 << PCD8544_CE_PIN;
	
	//RST pin as output
	PCD8544_RST_DDR |= 1 << PCD8544_RST_PIN;
	
	//BL pin as output
	PCD8544_BL_DDR |= 1 << PCD8544_BL_PIN;
...
void PCD8544_ClearRow(unsigned char y) {
	...
	for (i = 0; i < 6; i++) {
		...
		for (j = PCD8544_UpdateXmin; j <= PCD8544_UpdateXmax; j++) {
			PCD8544_Buffer[(i * PCD8544_WIDTH) + j] &= ~(1 << ((y-1) - i*8));
		}
	}
...
void PCD8544_SetArea(unsigned char yMin, unsigned char yMax, unsigned char xMin, unsigned char xMax, bool color) {
	unsigned char i, j, from, to, k, temp;
	for (i = 0; i < 6; i++) {
		//Not in range yet
		if (yMin > ((i + 1) * 8)) {
			continue;
		...
		for (j = xMin; j <= xMax; j++) {
			if (color) {
				PCD8544_Buffer[(i * PCD8544_WIDTH) + j] |= temp;
			} else{
				PCD8544_Buffer[(i * PCD8544_WIDTH) + j] &= ~temp;
			}
		}
...
void PCD8544_Putc(char c, PCD8544_Pixel_t color, PCD8544_FontSize_t size) {
	...
	for (i = 0; i < c_width - 1; i++) {
		if (c < 32) {
			if(size == PCD8544_FontSize_5x7){
			b = custom_chars._custom5x7[c][i];
			}
...
void PCD8544_Puts_f(const __flash char *c, PCD8544_Pixel_t color, PCD8544_FontSize_t size) {
	while (*c) {
		PCD8544_Putc(*c++, color, size);
	}
}
...
void PCD8544_PutSub_f(const __flash char *c, PCD8544_SubMenuRow_t row, PCD8544_Pixel_t color){
	...
	switch(row){
		case 1: PCD8544_GotoXY(X_ROW1, Y_ROW1);break;
